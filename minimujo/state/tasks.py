...
import numpy as np
from minimujo.entities import OBJECT_NAMES, ObjectEnum, get_color_id
...
def get_pickup_task(minigrid_env: MiniGridEnv, strict: bool = False):
    ...
    def pickup_task(prev_state: MinimujoState, cur_state: MinimujoState):
        for obj in cur_state.objects:
            obj_type = obj[MinimujoState.OBJECT_IDX_TYPE]
            # if is a held object
            if obj_type != ObjectEnum.DOOR.value and obj[MinimujoState.OBJECT_IDX_STATE] == 1:
                # if matches target
                if obj[MinimujoState.OBJECT_IDX_COLOR] == color_id and obj_type == obj_id:
                    return 1, True
                elif strict:
...
def get_put_near_task(minigrid_env: MiniGridEnv):
    ...
    pos_idx = MinimujoState.OBJECT_IDX_POS
    vel_idx = MinimujoState.OBJECT_IDX_VEL

    def put_near_task(prev_state: MinimujoState, cur_state: MinimujoState):
        for idx1, obj1 in enumerate(cur_state.objects):
            if obj1[MinimujoState.OBJECT_IDX_COLOR] != cidx1 \
                or obj1[MinimujoState.OBJECT_IDX_TYPE] != tidx1:
                continue
            ...
            for idx2, obj2 in enumerate(cur_state.objects):
                if idx1 == idx2:
                    continue
                if obj2[MinimujoState.OBJECT_IDX_COLOR] != cidx2 \
                    or obj2[MinimujoState.OBJECT_IDX_TYPE] != tidx2:
                    continue
