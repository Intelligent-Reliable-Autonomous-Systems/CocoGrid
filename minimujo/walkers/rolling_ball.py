...
class JumpingBallWithHead(legacy_base.Walker):
  ...
  def _build_observables(self):
    return RollingBallObservables(self)
...
class RollingBallObservables(legacy_base.WalkerObservables):

  @composer.observable
  def orientation(self):
    # rotation about z-axis scaled to [-1,1]
    def get_orientation(physics):
  ...
  def angular_velocity(self):
    ...
    def get_angular_velocity(physics):
      vel = physics.bind(self._entity.root_body).cvel
      return vel[2] / (2 * np.pi)
    return observable.Generic(get_angular_velocity)

  # @composer.observable
  # def orientation(self):
  #   def get_orientation(physics):
  #     qpos = physics.bind(self._entity.mjcf_model.find('joint', 'steer')).qpos
  #     print(qpos)
  #     return qpos
  #   return observable.Generic(get_orientation)
