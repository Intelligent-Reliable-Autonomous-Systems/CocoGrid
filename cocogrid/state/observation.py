...
class ObservationSpecification:
    ...
    def build_observation_space(self, state: CocogridState):
    ...
    def reset(self, state: CocogridState):
        if self._shuffle_objects:
            self._object_order = np.random.permutation(self._num_objects)

    def get_observe_object(self, object_idx):

        def observe_object(state: CocogridState):
    ...
    def get_observe_flat_features(self, flat_map: Dict[str, Tuple[int, int, Callable]], flat_dim: int):
        def observe_flat(state: CocogridState):
