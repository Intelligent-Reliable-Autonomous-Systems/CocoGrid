...
from cocogrid.common.color import COLOR_MAP
import numpy as np
from cocogrid.common.entity import ObjectEnum, get_color_id
from cocogrid.common.abstraction.grid_abstraction import GridAbstraction, hash_list_of_tuples, hash_ndarray
from cocogrid.common.cocogrid_state import CocogridState


@dataclass(frozen=True)
class RoomAbstraction:
    # constants
    ...
    def __post_init__(self):
        ...
        for idx, (obj_type, _, _) in enumerate(self.objects):
            if obj_type == RoomAbstraction.DOOR_IDX:
    ...
    def __repr__(self) -> str:
        obj_str = ",".join(map(RoomAbstraction.pretty_object, self.objects))
    ...
    def go_near_object(self, target_idx):
        ...
        # remove old nears
        filtered_pairs = tuple(pair for pair in self.nears if pair[0] != 0 and pair[1] != 0)
        ...
        return RoomAbstraction(self.objects, self.rooms, new_nears, _positions=self._positions)

    def move_away_from_objects(self):
        # remove old nears
        new_nears = tuple(pair for pair in self.nears if pair[0] != 0 and pair[1] != 0)
    ...
    def pick_up_object(self, target_idx):
        ...
        # remove all nears
        new_nears = tuple(pair for pair in self.nears if pair[0] != target_idx and pair[1] != target_idx)

        return RoomAbstraction(new_objects, self.rooms, new_nears, _positions=self._positions)
    ...
    def drop_object(self):
        ...
        new_nears = self.nears + tuple(tuple(sorted((idx, held_idx + 1))) for idx in near_agent)
    ...
    def open_door(self):
        ...
        near_doors = [
            idx_b - 1
            for idx_a, idx_b in self.nears
            if idx_a == 0 and self.objects[idx_b - 1][0] == RoomAbstraction.DOOR_IDX
        ]
        if len(near_doors) == 0:
            # not near a door
            return self
        ...
        if self.is_locked(near_door):
            ...
            if held_obj[0] != RoomAbstraction.KEY_IDX or held_obj[1] != self.objects[near_door][1]:
        ...
        return RoomAbstraction(new_objects, self.rooms, self.nears, _positions=self._positions)
    ...
    def go_through_door(self):
        ...
        moved_indices = [
            ...
            *[
                idx + 1
                for idx, (obj_type, _, state) in enumerate(self.objects)
        ...
        new_rooms = tuple(
            (
                tuple(val for i, val in enumerate(self.rooms[agent_room]) if val not in moved_indices)
                if idx == agent_room
                else tuple(self.rooms[connecting_room] + tuple(moved_indices))
                if idx == connecting_room
                else value  # Leave other tuples unchanged
            )
            for idx, value in enumerate(self.rooms)
        )

        return RoomAbstraction(self.objects, new_rooms, self.nears, _positions=self._positions)
    ...
    def can_hold(self, object_idx):
        ...
        return obj_type != RoomAbstraction.DOOR_IDX and obj_type != RoomAbstraction.GOAL_IDX
    ...
    @staticmethod
    def from_cocogrid_state(state: CocogridState):
        grid = GridAbstraction.from_cocogrid_state(state, force_door_evict=True)
        ...
        objects += ((RoomAbstraction.GOAL_IDX, get_color_id("green"), 0),) * (len(positions) - len(objects) - 1)
        ...
        def is_tile_type(idx):
            ...
            return idx != 0 and objects[idx - 1][0] == RoomAbstraction.GOAL_IDX
        ...
        def get_position(idx):
            if idx == 0:
                ...
                # object
                return state.objects[idx - 1, CocogridState.OBJECT_IDX_POS : CocogridState.OBJECT_IDX_POS + 2]
            else:
                return None

        def is_held(idx):
            return idx != 0 and objects[idx - 1][0] != RoomAbstraction.DOOR_IDX and objects[idx - 1][2] >= 1
        ...
        return RoomAbstraction(objects, rooms, nears, _positions=positions)
    ...
    def pretty_object(object_tuple: Tuple[int]):
        ...
        name = "unknown"
        if 0 <= oid < len(RoomAbstraction.OBJECT_IDS):
            name = RoomAbstraction.OBJECT_IDS[oid]
        color = list(COLOR_MAP.keys())[color_id]
        if oid == RoomAbstraction.DOOR_IDX:
            status = ["open", "closed", "locked", "locked"][state]
        elif oid == RoomAbstraction.GOAL_IDX:
            status = ""
        else:
...
def get_rooms_with_objects(grid_state: GridAbstraction):
    ...
    objects = [
        (x, y, idx + 1, obj_type == RoomAbstraction.DOOR_IDX)
        for idx, (obj_type, x, y, _, _) in enumerate(grid_state.objects)
    ]
    ...
    while len(regions_to_visit) > 0:
        ...
        for region_start in new_regions:
            ...
            if visited[region_start] == 1:
                continue
            if (
                grid_state.grid[region_start] == GridAbstraction.GRID_WALL
                or grid_state.grid[region_start] == GridAbstraction.GRID_LAVA
            ):
                continue
            ...
            while len(to_visit_in_region) > 0:
                ...
                if (
                    grid_state.grid[pos] == GridAbstraction.GRID_WALL
                    or grid_state.grid[pos] == GridAbstraction.GRID_LAVA
                ):
                    continue
