...
class DMCGym(Env):
    ...
    def _get_observation_space(self):
        observation_space, self.range_mapping, self.vector_dim = _spec_to_box_v3(
            self._env.observation_spec().values(), image_format=self.image_observation_format
        )
        ...
        def obs_func(timestep):
            ...
            if norm_image:
                observation = (observation / 255.0).astype(np.float32)
            return observation
    ...
    def step(self, action):
        ...
        termination = hasattr(self._env._task, "should_terminate_episode") and self._env._task.should_terminate_episode(
            self._env._physics_proxy
        )
...
class CocogridGym(DMCGym):
    def __init__(self, observation_spec=None, *args, **kwargs):
    ...
    def state(self) -> CocogridState:
        """Get the raw Cocogrid state representation"""
    ...
    @property
    def arena(self) -> CocogridArena:
        """Get the internal arena object"""
        return self._task._cocogrid_arena

    @property
    def minigrid(self) -> Env:
        """Get the minigrid environment"""
        return self._task._cocogrid_arena._minigrid
