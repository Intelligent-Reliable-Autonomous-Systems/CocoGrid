...
class CocogridArena(MazeArena):
    def __init__(
        ...
        super().__init__(maze=labmaze, xy_scale=xy_scale, z_height=z_height, name="cocogrid")
    ...
    def _build_observables(self):
        return CocogridObservables(self, self.cam_width, self.cam_height)
    ...
    def create_entity_mjcf(self):
        ...
        self._mini_entity_map = {Ball: [], Box: [], Door: [], Key: [], Goal: [], Lava: []}
        ...
        for idx, miniObj in enumerate(self._minigrid.grid.grid):
            if miniObj is not None:
                ...
                self._mini_entity_map[key].append(
                    {"entity": entity, "world_pos": world_pos, "mini_pos": (col, row), "mini_obj": miniObj}
                )
    ...
    def get_state_observer(self):
        filtered_entities = [
            entities for key, entities in self._mini_entity_map.items() if key in [Key, Ball, Box, Door]
        ]
        objects = [entity["entity"] for entity in itertools.chain(*filtered_entities)]
        return CocogridStateObserver(self._minigrid, self.xy_scale, objects, self._agent)
    ...
    def initialize_arena_mjcf(self, random_state=None):
        ...
        if not self._already_initialized:
            self.create_entity_mjcf()
            self.register_agent(self._agent)
    ...
    def register_agent(self, agent: "MuJoCoAgent"):
        self._agent = agent
        self._grabber.register_walker(agent.walker)
        for lava in self._mini_entity_map[Lava]:
            lava["entity"].register_walker(agent.walker)
        for goal in self._mini_entity_map[Goal]:
            goal["entity"].register_walker(agent.walker)
...
class CocogridObservables(composer.Observables):
